...
struct ImageUploadClient {
    func uploadImage(image: Data, index: String = "0", mimeType: String = "image/jpeg") async throws -> String? {
        ...
        printRequestDetails(request: request) // İstek detaylarını yazdır
        ...
        do {
            ...
            let responseModel = try JSONDecoder().decode(APICartonResponseModel.self, from: data)
            return responseModel.task_id
